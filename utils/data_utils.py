...
def load_adj_data(adj_pk_path, max_node_num, num_choice, emb_pk_path=None):
    ...
    #print(adj_concept_pairs[0][0].shape)
    if emb_pk_path is not None:
        with open(emb_pk_path, 'rb') as fin:
            all_embs = pickle.load(fin)
    ...
    #print(adj_data[0][0][1].shape)
    print(adj_lengths[0][0])
        ...
        for ex_index, example in enumerate(examples):
            ...
            for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
                ...
                if example.question != '':
                    ...
                    tokens_b = tokenizer.tokenize(ending, **extra_args)
    ...
    def _truncate_seq_pair(tokens_a, tokens_b, max_length):
        ...
        while True:
            ...
            if total_length <= max_length:
                break
            if len(tokens_a) > len(tokens_b):
                if '</s>' in tokens_a or '[SEP]' in tokens_a:
                    tokens_a = tokens_a[1:]
                else:
                    tokens_a = tokens_a[:-1]
            else:
...
def load_roberta_input_tensors_with_context(statement_jsonl_path, max_seq_length):
...
def load_input_tensors(input_jsonl_path, model_type, model_name, max_seq_length, format=[]):
    if 'aristo' in format:  # never used
        return load_roberta_input_tensors_with_context(input_jsonl_path, max_seq_length)
